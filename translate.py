...
for file_name in os.listdir(dir_path):
    ...
    if base_lang == 'pl':
        ...
        try:
            ...
            for k, v in pl_landing.items():
                if k.startswith('t_') and k.endswith('title') and k != 'themes_title':
                    translated_landing[k] = v
                    continue
                
                v_clean = v.replace('\n', ' [[NL]] ')
                try:
                    res = translator.translate(v_clean)
                    if res:
                        res = res.replace(' [[NL]] ', '\n').replace('[[NL]]', '\n')
                    ...
                    print(f"Error translating {k} to {g_lang}: {e}")
                    translated_landing[k] = v
        except Exception as e:
            print(f"Error initializing translator for {g_lang}: {e}")
            translated_landing = pl_landing
